...
get_stripe_count()
{
    ...
    if [[ $n_comp == 0 ]]; then
        stripes+=("$(lfs getstripe --stripe-count "$entry")")
    elif [[ $n_mirror > 1 ]]; then
        for i in $(seq 1 $n_mirror); do
            stripes+=("$(lfs getstripe --mirror-id=$i --stripe-count "$entry")")
        done
    else
        for i in $(seq 1 $n_comp); do
            stripes+=("$(lfs getstripe --component-id=$i --stripe-count "$entry")")
        done
    fi
...
verify_lustre()
{
    ...
    lfs getstripe -v "$entry"
    lfs getstripe -y "$entry"
    mongo $testdb --eval "db.entries.find()"
    echo "n_comp = '$n_comp'"
    echo "n_mirror = '$n_mirror'"

    echo "verification of lustre"
    ...
    # XXX: simple directories created with mkdir are not managed properly in the
    # Lustre backend, this is a temporary "fix" so that this function works for
    # every test but will be modified when the issue is resolved
    if [ "$hash_type" != "none" ]; then
        ...
        find_attribute '"xattrs.pattern":'$(get_pattern "$entry") \
                       '"ns.name":"'$name'"'
        find_attribute '"xattrs.stripe_count":'$(get_stripe_count "$entry" $n_comp $n_mirror) \
                       '"ns.name":"'$name'"'
        find_attribute '"xattrs.stripe_size":'$(get_stripe_size "$entry" $n_comp $n_mirror) \
                       '"ns.name":"'$name'"'
    fi
    echo "verification worked..."
}
