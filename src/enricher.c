...
#define XATTRS_VALUES_STACK_LENGTH \
    (MIN_XATTR_VALUES_ALLOC * sizeof(struct rbh_value *))
...
static __thread char lustre_path[PATH_MAX];
...
{
    struct lu_fid *fid = rbh_lu_fid_from_id(id);
    ...
    /* Allocate almost a full stack to try and get the path. If it is not
     * enough, we will use the static variable defined above, and pop the space
     * pushed on the stack.
     */
    path = rbh_sstack_push(xattrs_values, NULL, XATTRS_VALUES_STACK_LENGTH - 1);
    if (path == NULL)
        return -1;
     ...
     * preprare the append.
     */
    rc = llapi_fid2path_at(mount_fd, fid, path,
                           XATTRS_VALUES_STACK_LENGTH - 1 - strlen(name),
                           &recno, &linkno);
    value_path = path;
    if (rc && errno == ERANGE) {
        /* If llapi_fid2path_at failed with ERANGE, that means the length of the
         * array given wasn't enough. So we instead will use the static
         * variable. But before that, pop the space previously pushed.
         */
        rc = rbh_sstack_pop(xattrs_values, XATTRS_VALUES_STACK_LENGTH - 1);
        if (rc)
            return -1;

        rc = llapi_fid2path_at(mount_fd, fid, lustre_path,
                               PATH_MAX - strlen(name), &recno, &linkno);
        value_path = lustre_path;
    } else {
        /* remove the extra space left in the sstack */
        rc = rbh_sstack_pop(
            xattrs_values,
            XATTRS_VALUES_STACK_LENGTH - 1 - (strlen(value_path) + 1)
            );
    }
