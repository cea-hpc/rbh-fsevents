                    ...
                    struct rbh_value_pair *pair)
{
    ...
    char parent_fid[FID_LEN];
    char full_path[PATH_MAX];
    char fsname[PATH_MAX];
    int rc;

    rc = llapi_search_mounts(NULL, 0, fsname, NULL);
    if (rc == -1)
        return -1;

    rc = sprintf((char *)&parent_fid, DFID, PFID(&record->cr_pfid));
    if (rc == -1)
        return -1;

    rc = llapi_fid2path(fsname, parent_fid, parent_path, PATH_MAX, NULL, NULL);
    if (rc)
        return -1;

    /* If parent_path is the root of the Lustre FS, it's path is "/", otherwise
     * it is the normal path without a preceding or ending "/", so we must add
     * them.
     */
    rc = sprintf(full_path, "%s%s%s%s", parent_path[0] == '/' ? "" : "/",
                                        parent_path,
                                        parent_path[0] == '/' ? "" : "/",
                                        changelog_rec_name(record));
    if (rc < 0)
        return -1;

    path_value.type = RBH_VT_STRING;
    path_value.string = rbh_sstack_push(values, full_path, strlen(full_path) + 1);
    if (path_value.string == NULL)
        return -1;
                      ...
                      struct rbh_value_pair *pair)
{
    ...
    if (fill_ns_xattrs_path(values, record, &submap_pairs[0]))
        return -1;

    if (fill_ns_xattrs_fid(values, record, &submap_pairs[1]))
        return -1;
