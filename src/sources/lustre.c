...
_values_flush()
{
    while (true) {
        ...
        rbh_sstack_peek(_values, &readable);
        if (readable == 0)
            break;

        rbh_sstack_pop(_values, readable);
    }
    rbh_sstack_shrink(_values);
}
...
build_id(struct changelog_rec *record)
{
    ...
    case CL_CREATE:
        ...
        switch(records->process_step) {
            case 0:
                ...
                fsevent->link.parent_id = build_id(record);
