...
{
    error(EXIT_FAILURE, 0, "parser error: %s\n", parser->problem);
...
yaml_fsevent_iter_next(void *iterator)
{
    ...
    case YAML_DOCUMENT_START_EVENT:
        switch (fsevents->fsevent.type) {
        case RBH_FET_UPSERT:
            fsevents->fsevent.upsert.statx = NULL;
            fsevents->fsevent.upsert.symlink = NULL;
            break;
        case RBH_FET_LINK:
        case RBH_FET_UNLINK:
        case RBH_FET_XATTR:
            fsevents->fsevent.link.parent_id = NULL;
            fsevents->fsevent.link.name = NULL;
            break;
        default:
            break;
        }
    ...
    case YAML_STREAM_END_EVENT:
        ...
        return NULL;
    default:
        error(EXIT_FAILURE, 0, "unknown fsevent type: %i\n", event.type);
        __builtin_unreachable();
    }
}
