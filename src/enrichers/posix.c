...
posix_backend_enrich_process(void *_enrich, struct rbh_iterator *fsevents)
{
    struct backend_enrich *enrich = _enrich;

    return posix_iter_enrich(fsevents, enrich->mount_fd);
}
...
static const struct enrich_operations POSIX_BACKEND_ENRICH_OPS = {
    .process = posix_backend_enrich_process,
    .destroy = posix_backend_enrich_destroy,
};

static const struct enrich POSIX_BACKEND_ENRICH = {
    .name = "posix backend",
    .ops = &POSIX_BACKEND_ENRICH_OPS,
};
...
posix_enrich_from_backend(struct rbh_backend *backend, const char *mount_path)
{
    ...
    enrich->enrich = POSIX_BACKEND_ENRICH;
