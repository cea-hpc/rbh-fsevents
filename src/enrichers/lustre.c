...
static struct rbh_iterator *
lustre_backend_enrich_process(void *_enrich, struct rbh_iterator *fsevents)
{
    struct backend_enrich *enrich = _enrich;

    return lustre_iter_enrich(enrich->backend, fsevents, enrich->mount_fd);
}

static const struct enrich_operations LUSTRE_BACKEND_ENRICH_OPS = {
    .process = lustre_backend_enrich_process,
    .destroy = posix_backend_enrich_destroy,
};

static const struct enrich LUSTRE_BACKEND_ENRICH = {
    .name = "lustre backend",
    .ops = &LUSTRE_BACKEND_ENRICH_OPS,
};
...
lustre_enrich_from_backend(struct rbh_backend *backend, const char *mount_path)
{
    ...
    enrich->enrich = LUSTRE_BACKEND_ENRICH;
