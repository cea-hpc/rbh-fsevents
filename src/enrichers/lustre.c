...
static int
_enrich(struct enricher *enricher, const struct rbh_value_pair *attr,
    ...
    *enriched = *original;
    ...
    for (size_t i = 0; i < original->xattrs.count; i++) {
        ...
        for (size_t i = 0; i < partials->count; i++) {
            ...
            rc = _enrich(enricher, &partials->pairs[i], original);
            if (rc == -1)
                return -1;
            if (rc != 0) /* means we encounter a lustre attribute */
                continue;
