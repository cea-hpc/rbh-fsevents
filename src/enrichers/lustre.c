...
#include <fcntl.h>
#include <string.h>
#include <unistd.h> // to check

#include <sys/stat.h>
#include <sys/types.h>

#include <robinhood.h>
#include <robinhood/backends/lustre_tool.h>
...
//    size_t pair_count = enricher->pair_count;
    ...
    for (size_t i = 0; i < original->xattrs.count; i++) {
        ...
        for (i = 0; i < partials->count; ++i) {
            ...
            if (strcmp(inner_pair->key, "xattrs") || inner_pair->value->type != RBH_VT_SEQUENCE)
                continue;

            xattrs = inner_pair->value;
            for (i = 0; i < xattrs->sequence.count; ++i) {
                const struct rbh_value *inner_value = &xattrs->sequence.values[i];
