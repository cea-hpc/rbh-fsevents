...
static enum regex_field __attribute__((pure))
...
parse_statx_mapping(yaml_parser_t *parser, struct statx *statxbuf)
{
    ...
    while (true) {
        ...
        case SF_UID:
            ...
            success = parse_uid(parser, &statxbuf->stx_nlink);
            break;
        case SF_GID:
...
emit_xattr(yaml_emitter_t *emitter, const struct rbh_fsevent *xattr)
{
    if (xattr->ns.parent_id) {
        assert(xattr->ns.parent_id);
        return emit_ns_xattr(emitter, xattr);
    }
    assert(xattr->ns.parent_id == NULL);
