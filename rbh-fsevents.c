...
static struct enrich_iter_builder *
enrich_iter_builder_new(const char *uri)
{
    struct enrich_iter_builder *builder = enrich_iter_builder_from_uri(uri);

    if (builder == NULL)
        error(EXIT_FAILURE, errno, "enrich_from_uri");
     ...
     struct enrich_iter_builder *builder, bool allow_partials)
{
    ...
    while (true) {
        ...
        if (builder != NULL)
            fsevents = enrich_iter_builder_process(builder, fsevents);
...
main(int argc, char *argv[])
{
    ...
    while ((c = getopt_long(argc, argv, "e:hlr", LONG_OPTIONS, NULL)) != -1) {
        switch (c) {
        case 'e':
            enrich_builder = enrich_iter_builder_new(optarg);
